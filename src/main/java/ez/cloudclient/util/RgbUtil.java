...
public class RgbUtil {
    ...
    {
        if (float3 < 0.0f) { ++float3; }
        if (float3 > 1.0f) { --float3; }
        if (6.0f * float3 < 1.0f) { return float1 + (float2 - float1) * 6.0f * float3; }
        if (2.0f * float3 < 1.0f) { return float2; }
        if (3.0f * float3 < 2.0f) { return float1 + (float2 - float1) * 6.0f * (0.6666667f - float3); }
        return float1;
    }
    ...
    public static Color GetRainbowColor(float hue, float saturation, float lightness, final float alpha) {
        ...
        lightness /= 100.0f;
        float float1;
        if (lightness < 0.0) { float1 = lightness * (1.0f + saturation); }
        else { float1 = lightness + saturation - saturation * lightness; }
